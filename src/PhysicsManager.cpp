		...
		it != PhysicsManager::GetPhysicalObjects()->end(); it++)
	{
		if (*it != &obj)
		{
			...
			if (result.result)
			{
				...
				//obj.getRigidBody()->velocity = { 0,0 };
				//(*it)->getRigidBody()->velocity = { 0,0 };
			}
			...
			if (abs(result.normal.x) > 0.0001f)
			{
				...
				if (vel_along_normal < 0)
				{
					...
					const float mass_sum = obj.getRigidBody()->mass + (*it)->getRigidBody()->mass;
					
					const float impulse_length = (-(1 + e) * vel_along_normal)
						/ (obj.getRigidBody()->inverse_mass + (*it)->getRigidBody()->inverse_mass);
